      ...
      return (
          <Link href={`/blog/${latest.slug}`} scroll={true}>
            <a className={'relative w-full flex flex-col p-2 rounded-xl group transition transform'}>
              <PostLinkBlurImage src={latest.coverPhoto.url} alt={latest.title} />
              <div className={'pt-4'}>
    ...
    case 'project': {
      return (
          <a
              href={latest.svn_url}
              target={'_blank'}
              className={'w-full flex flex-col h-full border-2 p-4 rounded-xl hover:border-blue-400 group transition transform hover:scale-110" rel="noreferrer'} rel="noreferrer">
            <h1 className={'font-semibold text-xl dark:text-gray-200 text-gray-700 text-left truncate'}>
              {latest.name}
            </h1>
